...
function dateBoundsUtc(date: string) {
  ...
  const parts = new Intl.DateTimeFormat('en-US', { timeZone: 'Europe/Warsaw', timeZoneName: 'longOffset' }).formatToParts(probe);
  const offsetPart = parts.find((part) => part.type === 'timeZoneName')?.value ?? 'GMT+00:00';
  const match = offsetPart.match(/GMT([+-])(\d{2}):?(\d{2})?/);
  const sign = match?.[1] === '-' ? -1 : 1;
  const hours = Number(match?.[2] ?? 0); const minutes = Number(match?.[3] ?? 0);
...
function canonicalSport(key: string): SportKey { const bare = key.trim().toLowerCase(); if (bare === 'football' || bare === 'soccer') return 'soccer'; if (bare === 'basketball') return 'basketball'; if (bare === 'icehockey' || bare === 'hockey') return 'icehockey'; if (bare === 'baseball') return 'baseball'; if (bare === 'americanfootball' || bare === 'american-football' || bare === 'nfl') return 'americanfootball'; if (bare === 'tennis') return 'tennis'; if (bare === 'volleyball') return 'volleyball'; if (bare === 'golf') return 'golf'; if (bare === 'handball') return 'handball'; if (bare === 'rugby') return 'rugby'; if (bare === 'tabletennis' || bare === 'table-tennis') return 'tabletennis'; if (bare === 'darts') return 'darts'; if (bare === 'cricket') return 'cricket'; if (bare === 'aussierules' || bare === 'aussie-rules') return 'aussierules'; if (key.startsWith('basketball_')) return 'basketball'; if (key.startsWith('icehockey_')) return 'icehockey'; if (key.startsWith('baseball_')) return 'baseball'; if (key.startsWith('americanfootball_')) return 'americanfootball'; if (key.startsWith('tennis_')) return 'tennis'; if (key.startsWith('volleyball_')) return 'volleyball'; if (key.startsWith('golf_')) return 'golf'; if (key.startsWith('handball_')) return 'handball'; if (key.startsWith('rugby')) return 'rugby'; if (key.startsWith('tabletennis_')) return 'tabletennis'; if (key.startsWith('darts_')) return 'darts'; if (key.startsWith('cricket_')) return 'cricket'; if (key.startsWith('aussierules_')) return 'aussierules'; return 'soccer'; }
...
async function fetchOddsApiFallback(requestedDate: string, requestedSport: string, issues: DatasetResponse['issues']): Promise<{ events: SportEvent[]; snapshots: OddsSnapshot[]; availableSports: Array<{ key: string; title: string; group: string }>; queriedSports: string[]; bookmakers: string[]; quota?: DatasetResponse['quota'] }> {
  ...
  try {
    ...
    for (const wanted of requested) { const match = catalog.find((s) => s.active && (s.key === wanted || s.group.toLowerCase() === wanted.toLowerCase())); if (match && !sports.includes(match.key)) sports.push(match.key); }
  } catch (error) { issues.push({ code: 'odds-api-catalog-error', severity: 'warning', message: error instanceof Error ? error.message : 'The Odds API catalog failed.' }); return { events: [], snapshots: [], availableSports, queriedSports: sports, bookmakers: [] }; }
  const { from, to } = dateBoundsUtc(requestedDate); const events = new Map<string, SportEvent>(); const snapshots: OddsSnapshot[] = []; const bookmakers = new Map<string, string>(); let quota: DatasetResponse['quota'];
...
async function fetchSportScoreFallback(requestedDate: string, requestedSport: string, issues: DatasetResponse['issues']) {
  ...
  const results = await Promise.all(sports.map(async sport => {
    ...
    for (let attempt = 0; attempt < attempts.length; attempt += 1) {
      try { const url = new URL('https://sportscore.com/api/widget/matches/'); url.searchParams.set('sport', sport); url.searchParams.set('limit', '50'); if (attempts[attempt].src) url.searchParams.set('src', attempts[attempt].src); const response = await fetchWithTimeout(url, { headers: attempts[attempt].headers }, 8000); const raw = await response.text(); if (response.ok) { const payload = JSON.parse(raw) as { matches?: Array<Record<string, unknown>> }; return { sport, matches: Array.isArray(payload.matches) ? payload.matches : [] }; } lastError = `SportScore HTTP ${response.status}: ${raw.slice(0, 160)}`; if (![403,429,500,502,503,504].includes(response.status)) break; } catch (error) { lastError = error instanceof Error ? error.message : 'SportScore request failed.'; }
    }
    issues.push({ code: 'sportscore-error', severity: 'warning', message: lastError, reference: sport }); return { sport, matches: [] };
  }));
  const events: SportEvent[] = [];
  for (const r of results) for (const m of r.matches) { const home = String(m.home ?? m.home_team ?? m.homeTeam ?? '').trim(); const away = String(m.away ?? m.away_team ?? m.awayTeam ?? '').trim(); const t = String(m.time ?? m.start_time ?? m.startTime ?? m.commence_time ?? '').trim(); if (!home || !away || !t) continue; const d = new Date(t); if (!Number.isFinite(d.getTime()) || polishDate(d.toISOString()) !== requestedDate) continue; const competition = String(m.competition ?? m.league ?? m.tournament ?? labels[r.sport]).trim(); const status = String(m.status ?? m.state ?? '').toLowerCase(); events.push({ id:`sportscore:${r.sport}:${String(m.id ?? m.match_id ?? slug(home+'-'+away+'-'+t))}`, sportKey:canonicalSport(r.sport), league:{id:slug(competition),name:competition,sportKey:canonicalSport(r.sport),country:'International'}, homeTeam:team(home), awayTeam:team(away), startTime:d.toISOString(), status:status.includes('live')||status.includes('progress')?'live':status.includes('finish')||status.includes('ended')?'final':'scheduled', venue:'', monitored:true, liquidity:0.5 }); }
