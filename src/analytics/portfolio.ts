...
export interface PortfolioCandidate extends DependencySelection { odds: number; probability: number; ev?: number; qualityScore?: number; confidence?: number; }
export interface PortfolioConstraints { maxSelections?: number; maxSameEvent?: number; maxCorrelationGroup?: number; minEv?: number; minQuality?: number; minConfidence?: number; maxCombinedOdds?: number; targetCombinedOdds?: number; targetOddsTolerance?: number; }
...
function evaluateSelectionSet(selected: PortfolioCandidate[], constraints: PortfolioConstraints): PortfolioDecision | null {
  if (!selected.length || selected.length > (constraints.maxSelections ?? 8)) return null;
  ...
  if (selected.some((candidate) => (candidate.confidence ?? 1) < (constraints.minConfidence ?? 0))) return null;
  ...
  const odds = combinedOdds(selected.map((s) => s.odds)); if (odds > (constraints.maxCombinedOdds ?? Infinity)) return null;
...
function optimizeForTarget(ranked: PortfolioCandidate[], constraints: PortfolioConstraints): PortfolioDecision {
  const target = constraints.targetCombinedOdds!; const tolerance = Math.max(0, constraints.targetOddsTolerance ?? target * 0.10); const pool = ranked.slice(0, 24); let best: TargetBest | null = null;
  const visit = (start: number, selected: PortfolioCandidate[]): void => { if (selected.length > 0) { const decision = evaluateSelectionSet(selected, constraints); if (decision) { const distance = Math.abs(decision.combinedOdds - target); const inRange = distance <= tolerance; if (!best || (inRange && !best.inRange) || (inRange === best.inRange && (distance < best.distance || (distance === best.distance && decision.estimatedEv > best.decision.estimatedEv)))) best = { decision, inRange, distance }; } } if (selected.length >= (constraints.maxSelections ?? 8)) return; for (let i = start; i < pool.length; i += 1) visit(i + 1, [...selected, pool[i]]); };
  visit(0, []); if (!best) return { selected: [], rejected: ranked.map((candidate) => ({ id: candidate.id, reason: 'No portfolio satisfying target-odds constraints.' })), combinedOdds: 0, baseProbability: 0, adjustedProbability: 0, estimatedEv: 0, dependencyMultiplier: 1 };
  const winning = best as TargetBest; const selectedIds = new Set(winning.decision.selected.map((candidate) => candidate.id)); const rejected = ranked.filter((candidate) => !selectedIds.has(candidate.id)).map((candidate) => ({ id: candidate.id, reason: `Not selected by target-odds optimizer; target ${target.toFixed(2)} ± ${tolerance.toFixed(2)}.` })); return { ...winning.decision, rejected };
}
export function optimizePortfolio(candidates: PortfolioCandidate[], constraints: PortfolioConstraints = {}): PortfolioDecision {
  ...
  for (const candidate of ranked) {
    ...
    if ((candidate.confidence ?? 1) < (constraints.minConfidence ?? 0)) { rejected.push({ id: candidate.id, reason: 'Below minimum confidence constraint.' }); continue; }
    ...
    if (candidate.correlationGroup && selected.filter((s) => s.correlationGroup === candidate.correlationGroup).length >= maxGroup) { rejected.push({ id: candidate.id, reason: 'Correlation group limit reached.' }); continue; }
