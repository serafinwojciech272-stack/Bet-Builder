...
function conflictsWithSelected(candidate: PortfolioCandidate, selected: PortfolioCandidate[]): string | null {
  const deps = analyzeDependencies([...selected, candidate]);
  const conflict = deps.pairs.find((pair) => pair.type === 'MUTUALLY_EXCLUSIVE' && (pair.a === candidate.id || pair.b === candidate.id));
  return conflict ? `Mutually exclusive with ${conflict.a === candidate.id ? conflict.b : conflict.a}.` : null;
}
...
function evaluateSelectionSet(selected: PortfolioCandidate[], constraints: PortfolioConstraints): PortfolioDecision | null {
  ...
  const deps = analyzeDependencies(selected);
  const baseProbability = jointModelProbability(selected.map((s) => s.probability));
  ...
  return {
    ...
    baseProbability,
    adjustedProbability,
    estimatedEv: adjustedProbability * odds - 1,
    dependencyMultiplier: deps.adjustedJointProbabilityMultiplier,
  };
}

function optimizeForTarget(ranked: PortfolioCandidate[], constraints: PortfolioConstraints): PortfolioDecision {
  const target = constraints.targetCombinedOdds!;
  const tolerance = Math.max(0, constraints.targetOddsTolerance ?? target * 0.10);
  ...
  const visit = (start: number, selected: PortfolioCandidate[]): void => {
    if (selected.length > 0) {
      const decision = evaluateSelectionSet(selected, constraints);
      if (decision) {
        const distance = Math.abs(decision.combinedOdds - target);
        const inRange = distance <= tolerance;
        if (!best || (inRange && !best.inRange) || (inRange === best.inRange && (distance < best.distance || (distance === best.distance && decision.estimatedEv > best.decision.estimatedEv)))) {
          best = { decision, inRange, distance };
        }
      }
    }
  ...
  visit(0, []);
  if (!best) return { selected: [], rejected: ranked.map((candidate) => ({ id: candidate.id, reason: 'No portfolio satisfying target-odds constraints.' })), combinedOdds: 0, baseProbability: 0, adjustedProbability: 0, estimatedEv: 0, dependencyMultiplier: 1 };

  const selectedIds = new Set(best.decision.selected.map((candidate) => candidate.id));
  ...
  return { ...best.decision, rejected };
}

export function optimizePortfolio(candidates: PortfolioCandidate[], constraints: PortfolioConstraints = {}): PortfolioDecision {
  const maxSelections = constraints.maxSelections ?? 8;
  const maxSameEvent = constraints.maxSameEvent ?? 2;
  const maxGroup = constraints.maxCorrelationGroup ?? 2;
  const ranked = [...candidates]
    .filter((candidate) => Number.isFinite(candidate.odds) && candidate.odds > 1 && Number.isFinite(candidate.probability) && candidate.probability >= 0 && candidate.probability <= 1)
    .sort((a, b) => (b.ev ?? 0) - (a.ev ?? 0) || (b.qualityScore ?? 0) - (a.qualityScore ?? 0) || a.id.localeCompare(b.id));

  if (Number.isFinite(constraints.targetCombinedOdds) && constraints.targetCombinedOdds! > 1) {
    return optimizeForTarget(ranked, { ...constraints, maxSelections, maxSameEvent, maxCorrelationGroup: maxGroup });
  }
  ...
  for (const candidate of ranked) {
    ...
    const dependencyConflict = conflictsWithSelected(candidate, selected);
    if (dependencyConflict) { rejected.push({ id: candidate.id, reason: dependencyConflict }); continue; }
    const odds = combinedOdds([...selected, candidate].map((s) => s.odds));
