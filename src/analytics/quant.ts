 ...
 * The reported overround is the median bookmaker overround before normalization.
 */
export function marketEfficiency(snapshots: OddsSnapshot[]): MarketEfficiency | null {
  if (!snapshots.length) return null;
  const completeBooks: Array<{ normalized: Record<string, number>; overround: number }> = [];
  ...
  for (const snapshot of snapshots) {
    ...
    completeBooks.push({ normalized, overround: total - 1 });
  }
  ...
  for (const id of allSelections) {
    const values = completeBooks.filter((book) => book.normalized[id] !== undefined).map((book) => book.normalized[id]);
    if (values.length) normalizedProbabilities[id] = median(values);
  }
  ...
  return { marketId: snapshots[0].market, overround: median(completeBooks.map((book) => book.overround)), normalizedProbabilities, selections: Object.keys(normalizedProbabilities) };
}
...
export function buildQuantSignals(summaries: MarketQuoteSummary[], fairProbabilityBySelection: Record<string, number>, fairProbabilitySourceBySelection: Record<string, QuantSignal['fairProbabilitySource']> = {}): QuantSignal[] { return summaries.map((summary) => { const suppliedFair = fairProbabilityBySelection[summary.selectionId]; const fairProbability = clamp(suppliedFair ?? summary.impliedProbabilityBest); const value = fairProbability - summary.impliedProbabilityBest; const ev = modelEv(fairProbability, summary.bestOdds); const quality = qualityScore(summary.bookmakerCount, summary.dispersion); const confidence = clamp(0.45 * fairProbability + 0.35 * (quality / 100) + 0.2 * clamp(summary.bookmakerCount / 5)); const signal = quality < 45 ? 'LOW_QUALITY' : ev >= 0.08 && confidence >= 0.7 ? 'STRONG_VALUE' : ev >= 0.03 ? 'VALUE' : ev <= -0.03 ? 'NEGATIVE_VALUE' : 'NEUTRAL'; return { selectionId: summary.selectionId, odds: summary.bestOdds, impliedProbability: summary.impliedProbabilityBest, fairProbability, value, ev, confidence, qualityScore: quality, signal, fairProbabilitySource: fairProbabilitySourceBySelection[summary.selectionId] ?? 'MARKET_IMPLIED' }; }); }
...
export function eventQuantSnapshot(event: SportEvent, snapshots: OddsSnapshot[], fairProbabilityBySelection: Record<string, number> = {}): QuantSnapshot[] { const marketKeys = [...new Set(snapshots.filter((q) => q.eventId === event.id).map((q) => q.market))]; return marketKeys.map((marketKey) => { const marketSnapshots = snapshots.filter((q) => q.eventId === event.id && q.market === marketKey); const summaries = summarizeQuotes(marketSnapshots); const efficiency = marketEfficiency(marketSnapshots); const fair = { ...efficiency?.normalizedProbabilities, ...fairProbabilityBySelection }; const source = Object.fromEntries(summaries.map((summary) => [summary.selectionId, fairProbabilityBySelection[summary.selectionId] !== undefined ? 'MODEL' : efficiency ? 'DEVIG_CONSENSUS' : 'MARKET_IMPLIED'])) as Record<string, QuantSignal['fairProbabilitySource']>; return { eventId: event.id, marketKey, summaries, efficiency, signals: buildQuantSignals(summaries, fair, source) }; }); }
