...
export function createDecisionPacketFromAnalysis(analysis:AnalysisResponse,now=new Date(),selectionIds?:string[]):DecisionPacket{
  ...
  const decision=evaluateDecisionCenter(selections.map(s=>({id:s.id,marketId:s.marketId??analysis.context.market??'match-winner',eventId:s.eventId,name:s.id,shortName:s.id,odds:s.odds,probability:s.probability,impliedProbability:1/s.odds,value:s.probability-1/s.odds,ev:s.probability*s.odds-1,confidence:s.confidence,risk:s.risk as 'LOW'|'MEDIUM'|'HIGH'|'CRITICAL',correlationGroup:s.correlationGroup})));
