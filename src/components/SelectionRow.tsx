    ...
    <div className="rounded-lg border border-white/10 bg-white/[0.03] p-3">
      ...
      <div className="mt-3 grid grid-cols-3 gap-2 text-[11px] sm:grid-cols-5">
        <div>
          ...
          <div className="font-semibold text-white">{pct(analysis.probability)}</div>
        </div>
        <div>
          <div className="text-slate-500">Implied</div>
          <div className="font-semibold text-white">{pct(analysis.impliedProbability)}</div>
        </div>
        <div>
          <div className="text-slate-500">Value</div>
          <div className={`font-semibold ${analysis.value >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
            {analysis.value >= 0 ? '+' : ''}{pct(analysis.value)}
          </div>
        </div>
        <div>
          <div className="text-slate-500">EV</div>
          <div className={`font-semibold ${analysis.ev >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>{evPct}</div>
        </div>
        <div className="col-span-3 sm:col-span-1">
          <div className="text-slate-500">Quality</div>
          <div className="font-semibold text-white">{analysis.dataQuality}</div>
        </div>
      </div>
