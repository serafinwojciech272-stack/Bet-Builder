      ...
      {empty ? (
        ...
        <>
          ...
          <div className="space-y-3 border-t border-white/[.08] bg-black/10 p-4">
            ...
            <div className="flex items-center justify-center gap-1.5 pt-1"><span className={`live-dot h-1.5 w-1.5 rounded-full ${feedMode === 'LIVE' ? 'bg-emerald-400' : feedMode === 'DEMO' ? 'bg-amber-400' : 'bg-slate-500'}`} aria-hidden /><span className="text-[9px] font-bold uppercase tracking-[.16em] text-slate-600">{feedMode === 'LIVE' ? 'Live odds · Core Engine' : feedMode === 'DEMO' ? 'Demo data · Core Engine' : 'Feed unavailable · Core Engine'</span></div>
          </div>
        </>
      )}
    </aside>
