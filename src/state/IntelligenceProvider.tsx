...
export function IntelligenceProvider({ children }: { children: ReactNode }) {
  ...
  const seedPromiseRef = useRef<Promise<unknown> | null>(null); const analysisErrorRef = useRef<Record<string, string>>({}); const operator = (import.meta.env.VITE_OPERATOR_ID as string | undefined)?.trim() || 'operator';
  ...
  const runAnalysis = useCallback<IntelligenceValue['runAnalysis']>(async (eventId, options) => {
    ...
    try { analysisErrorRef.current[eventId] = ''; const analysis = await workspace.analysisService.analyzeEvent({ eventId, market: options?.market, depth: options?.depth ?? 'standard', requestedBy: operator }); await workspace.analysisRepository.save(analysis); await syncStores(); setAnalysisJobs((p) => ({ ...p, [eventId]: { status: 'idle' } })); pushToast({ tone: 'success', title: 'Analysis complete', detail: `${analysis.context.eventLabel} · confidence ${(analysis.confidence.score.value * 100).toFixed(0)}%` }); return analysis; }
    catch (e) { const message = e instanceof Error ? e.message : 'Analysis failed'; analysisErrorRef.current[eventId] = message; setAnalysisJobs((p) => ({ ...p, [eventId]: { status: 'error', error: message, retryable: e instanceof AnalysisError ? e.retryable : true } })); pushToast({ tone: 'error', title: 'Analysis failed', detail: message }); return null; }
  ...
  const toggleCheck = useCallback<IntelligenceValue['toggleCheck']>(async (missionId, checkId) => { await withMissionBusy(missionId, async () => { await workspace.missionService.toggleChecklistItem(missionId, checkId, operator); }); }, [workspace, withMissionBusy, operator]);
  const requestApproval = useCallback<IntelligenceValue['requestApproval']>(async (missionId) => withMissionBusy(missionId, async () => { await workspace.missionService.requestApproval(missionId, operator); pushToast({ tone: 'info', title: 'Sent to approval gate', detail: missionId }); }), [workspace, withMissionBusy, pushToast, operator]);
  const approveMission = useCallback<IntelligenceValue['approveMission']>(async (missionId, note) => withMissionBusy(missionId, async () => { await workspace.missionService.approve(missionId, operator, note); pushToast({ tone: 'success', title: 'Mission approved', detail: `${missionId} may now execute.` }); }), [workspace, withMissionBusy, pushToast, operator]);
  const rejectMission = useCallback<IntelligenceValue['rejectMission']>(async (missionId, note) => withMissionBusy(missionId, async () => { await workspace.missionService.reject(missionId, operator, note); pushToast({ tone: 'info', title: 'Mission rejected', detail: `${missionId} cancelled at the gate.` }); }), [workspace, withMissionBusy, pushToast, operator]);
  const cancelMission = useCallback<IntelligenceValue['cancelMission']>(async (missionId, reason) => withMissionBusy(missionId, async () => { await workspace.missionService.cancel(missionId, operator, reason); pushToast({ tone: 'info', title: 'Mission cancelled', detail: missionId }); }), [workspace, withMissionBusy, pushToast, operator]);
  const executeMission = useCallback<IntelligenceValue['executeMission']>(async (missionId) => withMissionBusy(missionId, async () => { const result = await workspace.missionService.execute(missionId, operator); pushToast({ tone: result.status === 'COMPLETED' ? 'success' : 'error', title: result.status === 'COMPLETED' ? 'Mission completed' : 'Mission failed', detail: result.measurement ? `Verdict ${result.measurement.verdict} · CLV ${result.measurement.closingLineValuePct.formatted}` : result.execution?.failureReason ?? undefined }); }), [workspace, withMissionBusy, pushToast, operator]);
