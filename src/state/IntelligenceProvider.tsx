...
export interface EngineStatus { aiEngineId: string; aiOnline: boolean; aiDetail: string; coreEngineId: string; coreVersion: string; monetaryExecution: boolean; }
...
export function IntelligenceProvider({ children }: { children: ReactNode }) {
  ...
  const syncStores = useCallback(async () => {
    const [nextAnalyses, nextMissions] = await Promise.all([workspace.analysisRepository.list(), workspace.missionRepository.list()]);
    setAnalyses(nextAnalyses); setMissions(nextMissions);
  }, [workspace]);

  const refresh = useCallback(async (date = selectedDate, forceRefresh = false) => {
    setPhase('loading'); setError(null); setSelectedDate(date);
    try {
      const data = await workspace.dataRepository.loadCanonicalDataset({ date, forceRefresh });
      setDataset(data);
      const [health, caps] = await Promise.all([workspace.analysisService.health(), workspace.coreEngine.capabilities()]);
      setEngine({ aiEngineId: health.engineId, aiOnline: health.ok, aiDetail: health.detail, coreEngineId: caps.engineId, coreVersion: caps.version, monetaryExecution: caps.supportsMonetaryExecution });
      if (!seedPromiseRef.current) seedPromiseRef.current = seedWorkspace(workspace);
      await seedPromiseRef.current;
      await syncStores();
      setLastRefreshedAt(new Date().toISOString()); setPhase('ready');
    } catch (e) {
      setError(e instanceof Error ? e.message : 'Unknown bootstrap failure'); setPhase('error');
    }
  ...
  const runAnalysis = useCallback<IntelligenceValue['runAnalysis']>(async (eventId, options) => {
    ...
    try {
      const analysis = await workspace.analysisService.analyzeEvent({ eventId, market: options?.market, depth: options?.depth ?? 'standard', requestedBy: operator });
      await workspace.analysisRepository.save(analysis); await syncStores();
      setAnalysisJobs((prev) => ({ ...prev, [eventId]: { status: 'idle' } }));
      pushToast({ tone: 'success', title: 'Analysis complete', detail: `${analysis.context.eventLabel} · confidence ${(analysis.confidence.score.value * 100).toFixed(0)}%` });
      return analysis;
    } catch (e) {
      const message = e instanceof Error ? e.message : 'Analysis failed';
      setAnalysisJobs((prev) => ({ ...prev, [eventId]: { status: 'error', error: message, retryable: e instanceof AnalysisError ? e.retryable : true } }));
      pushToast({ tone: 'error', title: 'Analysis failed', detail: message }); return null;
    }
  ...
  const withMissionBusy = useCallback(async (missionId: string, fn: () => Promise<void>) => {
    ...
    try { await fn(); } catch (e) {
      const message = e instanceof MissionTransitionError ? `${e.code}: ${e.message}` : e instanceof Error ? e.message : 'Mission operation failed';
      pushToast({ tone: 'error', title: 'Mission blocked', detail: message });
    } finally { setBusyMissionIds((prev) => prev.filter((id) => id !== missionId)); await syncStores(); }
  ...
  const toggleCheck = useCallback<IntelligenceValue['toggleCheck']>(async (missionId, checkId) => withMissionBusy(missionId, () => workspace.missionService.toggleChecklistItem(missionId, checkId, operator)), [workspace, withMissionBusy]);
  ...
  const value = useMemo<IntelligenceValue>(() => ({ phase, error, dataset, analyses, missions, engine, analysisJobs, busyMissionIds, toasts, lastRefreshedAt, selectedDate, nowTick, operator, refresh, runAnalysis, createMission, toggleCheck, requestApproval, approveMission, rejectMission, cancelMission, executeMission, simulateEngineFailure, dismissToast }), [phase, error, dataset, analyses, missions, engine, analysisJobs, busyMissionIds, toasts, lastRefreshedAt, selectedDate, nowTick, refresh, runAnalysis, createMission, toggleCheck, requestApproval, approveMission, rejectMission, cancelMission, executeMission, simulateEngineFailure, dismissToast]);
