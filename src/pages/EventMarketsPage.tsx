...
export default function EventMarketsPage({ addSelection, removeSelection, isAdded }: Props) {
  ...
  const [grouped, setGrouped] = useState<Record<string, typeof markets>>({});
  ...
  if (!event) {
    return (
      <main className="mx-auto max-w-[1400px] px-4 py-10">
        <div className="rounded-lg border border-white/10 p-10 text-center text-slate-300">
          Event not found.
        </div>
        <button onClick={() => navigate('/sports')} className="mt-4 text-violet-400">
          ← Back to Sports
        </button>
      </main>
    );
  ...
  return (
    <main className="mx-auto w-full max-w-[1400px] px-4 py-6 pb-24 md:pb-10">
      <button onClick={() => navigate('/sports')} className="text-violet-400 text-sm">
        ← Back to Sports
      </button>
      <div className="mt-3 flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <div className="mt-1 text-xs text-slate-400">
            {event.status === 'LIVE' ? 'LIVE' : time} · {event.status}
          </div>
        </div>
        <DemoBadge />
      </div>
      ...
      <div className="mt-6 space-y-8">
        ...
        {sections.length === 0 ? (
          <div className="rounded-lg border border-white/10 p-10 text-center text-slate-400">
            No open markets for this event (demo).
          </div>
        ) : null}
      </div>
    </main>
