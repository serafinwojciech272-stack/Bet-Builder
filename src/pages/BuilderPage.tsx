...
export default function BuilderPage({ selections, stake, addSelection, removeSelection, clear, updateStake }: Props) {
  ...
  const onOptimize = async () => {
    ...
    if (result) setMessage(`Core Engine: ${result.selections.length} selection(s) retained, ${result.rejectedSelections.length} filtered · diversification ${(result.diversificationScore * 100).toFixed(0)}%.`);
  };
    ...
    <main className="mx-auto w-full max-w-[1400px] px-4 py-6 pb-28 md:pb-10">
      ...
      <div className="mt-5 grid gap-5 lg:grid-cols-[1fr_360px]">
        <div className="space-y-5">
          <section aria-label="Events"><h2 className="text-[12px] font-bold uppercase tracking-[0.2em] text-slate-400 border-b border-white/10 pb-2">Live Events</h2><div className="mt-3 space-y-2">
            {events.map((e) => <div key={e.id} className="rounded-lg border border-white/10 bg-white/[0.02] p-3"><div className="flex items-center justify-between gap-2"><div className="min-w-0"><div className="text-sm font-bold text-white truncate">{e.homeTeam} vs {e.awayTeam}</div><div className="text-[11px] uppercase tracking-wider text-slate-400">{e.sport} · {e.league}</div></div><div className="text-[11px] text-slate-500">{new Date(e.startTime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</div></div><div className="mt-2 flex flex-wrap gap-1.5">
              {e.markets.flatMap((m) => m.selections).map((s) => { const added = isAdded(s.id); return <button key={s.id} type="button" aria-pressed={added} onClick={() => (added ? removeSelection(s.id) : addSelection(s))} className={`rounded-full border px-3 py-1.5 text-xs font-semibold transition ${added ? 'border-violet-500/60 bg-violet-500/20 text-violet-100' : 'border-white/15 bg-white/5 text-slate-300 hover:border-violet-500/40 hover:text-white'}`}>{s.shortName} {s.odds.toFixed(2)}</button>; })}
            </div></div>)}
