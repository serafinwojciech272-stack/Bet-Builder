...
export default function BuilderPage({ selections, stake, addSelection, removeSelection, clear, updateStake }: Props) {
  ...
  <div className="flex flex-wrap items-center gap-2">
    {['all', ...Array.from(new Set(allEvents.map((e) => e.sport)))].map((sport) => (
      <button key={sport} type="button" onClick={() => setSportFilter(sport)} className={`rounded-full px-3 py-1.5 text-[10px] font-bold ${sportFilter === sport ? 'bg-violet-500/20 text-violet-100 ring-1 ring-violet-400/40' : 'bg-white/[.03] text-slate-500 ring-1 ring-white/10'}`}>
        {sport === 'all' ? 'All sports' : sport === 'americanfootball' ? 'Football' : sport === 'icehockey' ? 'Ice Hockey' : sport === 'tabletennis' ? 'Table Tennis' : sport}
      </button>
    ))}
    <span className="ml-auto font-mono text-[9px] uppercase tracking-[.14em] text-slate-600">{events.length} events</span>
  </div>
</div>
