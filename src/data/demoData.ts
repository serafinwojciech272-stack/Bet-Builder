import type { Market, OddsSnapshot, Selection } from '../domain/types';
...
export function buildSelection(
  ...
  shortName: string,
  odds: number,
  opts?: { line?: number; correlationGroup?: string; seed?: string },
): Selection {
  const seed = opts?.seed ?? id;
  const implied = impliedProbability(odds);
  // model probability: deterministic offset from implied, scaled by skill-ish factor
...
export function buildOddsHistory(selectionId: string, currentOdds: number): OddsSnapshot[] {
  // opening = slightly different; 5 snapshots
  const open = Math.max(1.05, currentOdds + (stableRand(selectionId + 'o') - 0.5) * 0.3);
  const snaps: OddsSnapshot[] = [];
  for (let i = 0; i < 5; i++) {
    const frac = i / 4;
    const odds = open + (currentOdds - open) * frac;
    snaps.push({
      id: `${selectionId}-snap-${i}`,
      selectionId,
      odds: Math.round(odds * 100) / 100,
      timestamp: iso((4 - i) * 3600_000),
      provider: 'DEMO-ODDS',
      sourceQuality: 'HIGH',
    });
  }
...
export function buildMarketsForEvent(eventId: string, specs: DemoMarketSpec[]): { markets: Market[]; selections: Selection[]; histories: Record<string, OddsSnapshot[]> } {
  ...
  for (const spec of specs) {
    ...
    markets.push({
      id: marketId,
      eventId,
      type: spec.market.type,
      name: spec.market.name,
      category: spec.market.category,
      status: 'OPEN',
    });
    for (const s of spec.selections) {
      const selId = `${marketId}-${s.shortName.replace(/\s+/g, '').toLowerCase()}`;
      const selection = buildSelection(selId, marketId, eventId, s.name, s.shortName, s.odds, {
        line: s.line,
