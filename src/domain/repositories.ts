...
function deriveProviderHealth(data: CanonicalDataset): ProviderHealth {
  const fetchedAt = data.normalizedAt; const ageSeconds = Math.max(0, Math.round((Date.now() - new Date(fetchedAt).getTime()) / 1000)); const staleAfterSeconds = 600;
  const queriedSports = data.sportsQueried?.length ?? 0; const failedSports = data.issues.filter((i) => i.code === 'provider-error' || i.code === 'provider-network-error' || i.code === 'provider-payload-error').length; const successfulSports = Math.max(0, queriedSports - failedSports);
...
export class MockSportsDataRepository implements SportsDataRepository {
  private cache: CanonicalDataset | null = null; private attempts = 0;
  ...
  async loadCanonicalDataset(): Promise<CanonicalDataset> {
    if (this.cache) return this.cache; await delay(this.options.latencyMs ?? 520); this.attempts += 1;
    ...
    const eventResult = normalizeEvents(RAW_EVENTS); const ids = new Set(eventResult.value.map((e) => e.id)); const oddsResult = normalizeOddsSnapshots(RAW_ODDS, ids);
    this.cache = { events: eventResult.value, snapshots: oddsResult.value, issues: [...eventResult.issues, ...oddsResult.issues], droppedRecords: eventResult.droppedRecords + oddsResult.droppedRecords, normalizedAt: new Date().toISOString(), provider: 'demo', mode: 'DEMO' }; return this.cache;
  }
...
export class LiveSportsDataRepository implements SportsDataRepository {
  private cache = new Map<string, CanonicalDataset>(); private readonly testFallback = import.meta.env.MODE === 'test'; private readonly testRepository = new MockSportsDataRepository({ latencyMs: 0 });
  async loadCanonicalDataset(options: { date?: string; sport?: string; forceRefresh?: boolean } = {}): Promise<CanonicalDataset> {
    ...
    try {
      ...
      if (!data.events.length) {
        const fallback = await this.testRepository.loadCanonicalDataset(); const providerIssues = data.issues ?? [];
        const degraded: CanonicalDataset = { ...fallback, normalizedAt: new Date().toISOString(), requestedDate: date, provider: 'demo', mode: 'DEMO', availableSports: data.availableSports, quota: data.quota, issues: [{ code: 'live-provider-empty', severity: 'warning', message: providerIssues[0]?.message ?? 'Live provider returned no events. Showing deterministic fallback data.' }, ...providerIssues, ...fallback.issues] };
        this.cache.set(cacheKey, degraded); return degraded;
      }
      const normalized: CanonicalDataset = { ...data, provider: 'parlay-api', mode: 'LIVE', requestedDate: date }; normalized.providerHealth = deriveProviderHealth(normalized); this.cache.set(cacheKey, normalized); return normalized;
    } catch (error) {
      const fallback = await this.testRepository.loadCanonicalDataset(); const detail = error instanceof Error ? error.message : 'Unknown provider failure';
      const degraded: CanonicalDataset = { ...fallback, normalizedAt: new Date().toISOString(), requestedDate: date, provider: 'demo', mode: 'DEMO', availableSports: undefined, quota: undefined, issues: [{ code: 'live-provider-fallback', severity: 'warning', message: 'Live odds unavailable (' + detail + '). Showing deterministic fallback data.' }, ...fallback.issues] };
      this.cache.set(cacheKey, degraded); return degraded;
    }
  }
  async getEvent(eventId: string): Promise<SportEvent | null> { const data = await this.loadCanonicalDataset(); return data.events.find((e) => e.id === eventId) ?? null; }
