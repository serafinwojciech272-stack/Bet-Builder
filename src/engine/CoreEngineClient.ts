...
export interface CoreEngineCapabilities {
  ...
  /** Always false in this build — no real-money capability exists. */
...
/**
 * Port to the (future) Core Engine. Everything downstream of the approval gate
 * — execution, measurement, learning — goes through this interface, so the mock
 * can be swapped for a network client without touching the app.
 */
...
export class MockCoreEngineClient implements CoreEngineClient {
  ...
  async capabilities(): Promise<CoreEngineCapabilities> {
    ...
    return {
      ...
      supportsMonetaryExecution: false,
      notes:
        'Simulated Core Engine. Execution is observational only; the interface is stable for a real engine.',
    };
  }
  ...
  async submitMission(mission: Mission): Promise<CoreEngineAck> {
    ...
    if (mission.approval.state !== 'APPROVED') {
      throw new CoreEngineError(
        'APPROVAL_REQUIRED',
        'Core Engine rejected the mission: approval gate not satisfied.',
      );
    }
    const unsupported = mission.actions.find(
      (a) => !['OBSERVE_MARKET', 'CAPTURE_SNAPSHOT', 'EVALUATE_TRIGGER', 'RECOMPUTE_ANALYSIS', 'RAISE_ALERT', 'REPORT'].includes(a.kind),
    );
    if (unsupported) {
      throw new CoreEngineError(
        'UNSUPPORTED_ACTION',
        `Action ${unsupported.kind} is not permitted by this engine.`,
      );
    }
    return {
      accepted: true,
      engineRef: `CE-${mission.id.slice(-8)}-${Date.now().toString(36).toUpperCase()}`,
  ...
  async executeMission(mission: Mission, analysis: AnalysisResponse | null): Promise<ExecutionInfo> {
    ...
    const observation = analysis?.marketObservations.find(
      (o) => o.selectionId === mission.target.selectionId,
    ) ?? analysis?.marketObservations[0] ?? null;

    const threshold =
      mission.actions.find((a) => a.kind === 'EVALUATE_TRIGGER')?.trigger?.threshold.value ?? 3;
    ...
    const plan: Array<Pick<ExecutionStep, 'actionId' | 'label' | 'status' | 'detail'>> = [
      {
        actionId: 'act-observe',
        label: 'Observation window opened',
        status: 'ok',
        detail: `Watching ${mission.target.selectionLabel ?? mission.target.marketLabel} across ${mission.target.bookmakers.length} book(s).`,
      },
      {
        actionId: 'act-capture',
        label: 'Snapshots captured',
        status: analysis?.dataQuality.stale ? 'warning' : 'ok',
        detail: analysis?.dataQuality.stale
          ? 'Captures succeeded but the upstream feed is stale; marked as degraded input.'
          : `Canonical snapshots captured and normalized without loss.`,
      },
      {
        actionId: 'act-trigger',
        label: triggerFired ? 'Trigger fired' : 'Trigger evaluated — not fired',
        status: triggerFired ? 'warning' : 'ok',
        detail: `Observed |Δ| ${observedMove.toFixed(2)}% against threshold ${threshold.toFixed(1)}%.`,
      },
      {
        actionId: 'act-recompute',
        label: triggerFired ? 'Reassessment requested' : 'Reassessment not required',
        status: 'ok',
        detail: triggerFired
          ? 'Fresh intelligence pass queued for this market.'
          : 'Movement stayed inside the defined band; no reassessment queued.',
      },
      {
        actionId: 'act-alert',
        label: triggerFired ? 'Operator alert raised' : 'No alert necessary',
        status: 'ok',
        detail: triggerFired
          ? 'Alert emitted to the intelligence dashboard.'
          : 'Silent completion — nothing actionable observed.',
      },
      {
        actionId: 'act-report',
        label: 'Observation window closed',
        status: 'ok',
        detail: 'Handing off to measurement.',
      },
    ];
    ...
    return {
      ...
      engineRef: ack.engineRef,
      steps,
      observationsCount: det(steps.length, 'count', 'measurement-service'),
      triggerFired,
  ...
  async measureMission(mission: Mission, analysis: AnalysisResponse | null): Promise<MeasurementInfo> {
    ...
    const result = measure({
      modelProbability: estimate?.modelProbability.value ?? 0.5,
    ...
    const objectiveMet =
      mission.expectedOutcome.measurableMetric === 'data-quality-score'
    ...
    return {
      ...
      objectiveMet,
      notes: [
        `Reference implied probability at analysis: ${(impliedAtAnalysis * 100).toFixed(1)}%.`,
        `Measured implied probability at close: ${(impliedAtClose * 100).toFixed(1)}%.`,
        objectiveMet
          ? 'Mission objective met within the defined horizon.'
          : 'Mission objective not met — movement stayed inside the target band.',
      ],
    };
  }
  ...
  async publishLearning(mission: Mission): Promise<{ accepted: boolean; lessons: string[] }> {
    ...
    if (mission.measurement) {
      lessons.push(
        mission.measurement.verdict === 'beat-close'
      ...
      if (!mission.measurement.objectiveMet) {
        lessons.push('Trigger threshold may be too wide for this liquidity profile.');
      }
    }
    if (mission.execution?.steps.some((s) => s.status === 'warning')) {
      lessons.push('Degraded inputs observed during execution — raise data-quality gate for similar missions.');
    }
