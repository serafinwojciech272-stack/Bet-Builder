...
export class MockCoreEngineClient implements CoreEngineClient {
  ...
  async measureMission(mission: Mission, analysis: AnalysisResponse | null): Promise<MeasurementInfo> {
    ...
    return { measuredAt: new Date().toISOString(), verdict: result.verdict, closingLineValuePct: result.closingLineValuePct, realizedMovementPct: result.realizedMovementPct, calibrationDeltaPct: result.calibrationDeltaPct, brierScore: result.brierScore, objectiveMet, notes: [`Reference implied probability at analysis: ${(impliedAtAnalysis * 100).toFixed(1)}%.`, `Measured implied probability at close: ${(impliedAtClose * 100).toFixed(1)}%.`, objectiveMet ? 'Mission objective met within the defined horizon.' : 'Mission objective not met — movement stayed inside the target band.'] };
  }
