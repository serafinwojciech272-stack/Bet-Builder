import type { EventResearch, ResearchFinding, ResearchLanguage, ResearchQuery, ResearchSource, ResearchSourceKind } from './types.js';
...
export function synthesizeResearch(eventId:string, queries:ResearchQuery[], sources:ResearchSource[], now=new Date()): EventResearch {
  ...
  for(const [category,ss] of grouped){
    ...
    const ages=ss.map(s=>s.publishedAt?new Date(s.publishedAt).getTime():NaN).filter(t=>Number.isFinite(t)).map(t=>Math.max(0,(now.getTime()-t)/36e5));
    const freshnessHours=ages.length?Math.min(168,...ages):168;
    findings.push({id:`RF-${hash(eventId+category+ss.map(s=>s.url).join('|'))}`,category:category as ResearchFinding['category'],statement:`${category} evidence aggregated from ${ss.length} source(s); signal is ${polarity}.`,polarity,confidence:clamp(.35+Math.min(.5,ss.length*.08)+(new Set(ss.map(s=>s.publisher)).size>1?.12:0)),sourceIds:ss.slice(0,8).map(s=>s.url),freshnessHours,independentSourceCount:new Set(ss.map(s=>s.publisher)).size});
  }
