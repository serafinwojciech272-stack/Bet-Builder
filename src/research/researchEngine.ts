...
export function synthesizeResearch(eventId:string, queries:ResearchQuery[], sources:ResearchSource[], now=new Date()): EventResearch {
  ...
  for(const [category,ss] of grouped){
    ...
    findings.push({id:`RF-${hash(eventId+category+ss.map(s=>s.url).join('|'))}`,category:category as ResearchFinding['category'],statement:`${category} evidence aggregated from ${ss.length} source(s); signal is ${polarity}.`,polarity,confidence:clamp(.35+Math.min(.5,ss.length*.08)+(new Set(ss.map(s=>s.publisher)).size>1?.12:0)),sourceIds:ss.slice(0,8).map(s=>s.url),freshnessHours:ss.filter(s=>s.publishedAt).length?Math.min(168,...ss.filter(s=>s.publishedAt).map(s=>Math.max(0,(now.getTime()-new Date(s.publishedAt!).getTime())/36e5))):168,independentSourceCount:new Set(ss.map(s=>s.publisher)).size});
  }
